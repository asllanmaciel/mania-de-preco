...
class LaunchPreflight
{
    ...
    private function grupoCobranca(array $metricas): array
    {
        return [
            'codigo' => 'cobranca',
            'titulo' => 'Cobrança',
            'descricao' => 'Itens mínimos para vender planos e refletir eventos do provedor.',
            'icone' => 'credit-card',
            'itens' => [
                $this->item('Asaas como provedor principal', 'O MVP está orientado para Asaas primeiro.', config('billing.default_provider') === 'asaas', true, 'Definir BILLING_PROVIDER=asaas', 'BILLING_PROVIDER', (string) config('billing.default_provider')),
                $this->item('ASAAS_API_KEY configurada', 'Necessária para criar clientes, assinaturas e cobranças.', filled(config('billing.providers.asaas.api_key')), true, 'Configurar ASAAS_API_KEY', 'ASAAS_API_KEY', filled(config('billing.providers.asaas.api_key')) ? 'definida' : 'ausente'),
                $this->item('ASAAS_WEBHOOK_TOKEN configurado', 'Necessário para validar eventos recebidos do Asaas.', filled(config('billing.providers.asaas.webhook_token')), true, 'Configurar ASAAS_WEBHOOK_TOKEN', 'ASAAS_WEBHOOK_TOKEN', filled(config('billing.providers.asaas.webhook_token')) ? 'definido' : 'ausente'),
                $this->item('Webhook Asaas registrado', 'A rota de recebimento precisa estar publicada na API.', Route::has('billing.webhooks.asaas'), true, 'Validar rota de webhook', null, Route::has('billing.webhooks.asaas') ? route('billing.webhooks.asaas') : 'ausente'),
